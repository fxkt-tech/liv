...
- [ ] 小组合空间完整遍历后，每个组合只出现一次且正确返回 Exhausted。
- [ ] 相同种子结果完全一致，不同种子可以改变顺序但不改变无约束全集。
- [ ] 高权重候选在统计测试中更倾向于较早出现，同时所有候选仍可到达。
- [ ] BudgetExceeded 不等同于 Exhausted，继续调用可以恢复扫描。
- [ ] 约束拒绝不会更新接受历史，约束错误会中止调用并保留一致状态。
- [ ] MaxSimilarity、MaxVideoAssetUses 和 MaxBGMUses 均有边界测试。
- [ ] 不可行转场/素材组合不会生成 Project，并记录稳定拒绝原因。
- [ ] 生成 Project 的时长、转场重叠、BGM 和全局图层绝对时间正确。
- [ ] 并发调用得到明确错误或被文档化的竞态保护，不产生调度相关结果。
